...
namespace UHack.Core
{
    ...
    public partial class WebHelper : IWebHelper
    {
        ...
        public WebHelper(HttpContextBase httpContext, ICacheManager cacheManager)
        {
        ...
        public virtual string GetThisPageUrl(bool includeQueryString, bool useSsl)
        {
            ...
            {
                string applicationHost = GetApplicationHost(useSsl);
                if (applicationHost.EndsWith("/"))
                    applicationHost = applicationHost.Substring(0, applicationHost.Length - 1);
                url = applicationHost + _httpContext.Request.RawUrl;
            }
            else
            {
        ...
        /// <summary>
        /// Gets application host location
        /// </summary>
        /// <param name="useSsl">Use SSL</param>
        /// <returns>Application host location</returns>
        public virtual string GetApplicationHost(bool useSsl)
        {
            var result = "";
            ...
            if (DataSettingsHelper.DatabaseIsInstalled())
            {
                ...
                var applicationContext = EngineContext.Current.Resolve<IApplicationContext>();
                var currentApplication = applicationContext.CurrentApplication;
                if (currentApplication == null)
                    throw new Exception("Current application cannot be loaded");
                ...
                if (String.IsNullOrWhiteSpace(httpHost))
                {
                    ...
                    //in this case use URL of a application entity configured in admin area
                ...
                if (useSsl)
                {
                    if (!String.IsNullOrWhiteSpace(currentApplication.SecureUrl))
                    {
                        //Secure URL specified. 
                        //So a application owner don't want it to be detected automatically.
                        //In this case let's use the specified secure URL
                        result = currentApplication.SecureUrl;
                    }
                    else
                    {
                        //Secure URL is not specified.
                        //So a application owner wants it to be detected automatically.
                ...
                else
                {
                    ...
                    {
                        //SSL is enabled in this application and secure URL is specified.
                        //So a application owner don't want it to be detected automatically.
        ...
        public virtual string GetApplicationLocation()
        {
            bool useSsl = IsCurrentConnectionSecured();
            return GetApplicationLocation(useSsl);
        }
        ...
        /// <param name="useSsl">Use SSL</param>
        /// <returns>Application location</returns>
        public virtual string GetApplicationLocation(bool useSsl)
        {
            //return HostingEnvironment.ApplicationVirtualPath;

            string result = GetApplicationHost(useSsl);
            if (result.EndsWith("/"))
        ...
        public virtual void RestartAppDomain(bool makeRedirect = false, string redirectUrl = "")
        {
            ...
            else
            {
                ...
                if (!success)
                {
                    throw new AppException("Proofc needs to be restarted due to a configuration change, but was unable to do so." + Environment.NewLine +
        ...
        public virtual bool IsPostBeingDone
        {
            get
            {
                if (_httpContext.Items["Proofc.IsPOSTBeingDone"] == null)
                    return false;
                return Convert.ToBoolean(_httpContext.Items["nop.IsPOSTBeingDone"]);
            }
            set
            {
                _httpContext.Items["Proofc.IsPOSTBeingDone"] = value;
            }
        }
